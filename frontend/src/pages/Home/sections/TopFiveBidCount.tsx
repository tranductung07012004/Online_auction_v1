...
import { getTop5MostBidCount, type ProductResponseFromAPI } from '../../../api/product';
import { useNavigate } from 'react-router-dom';

const TopFiveBidCount: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await getTop5MostBidCount();
      ...
      <Container maxWidth="lg">
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            >
              Top 5 products that have highest bid counts
            </Typography>
            <Box
              sx={{
                color: 'white',
...
export default TopFiveBidCount;
